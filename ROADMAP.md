...
```mermaid
flowchart TD
    subgraph v020 ["v0.2.0 Foundation Architecture"]
        M1["1. Scoped Companion Asset Harvesting"]
        M2["2. Strict AI Model Payload Gatekeeping"]
        M3["3. Companion Triplet Pre-Flight Handshake"]
        M1 --> M2 --> M3
    end
    
    subgraph v030 ["v0.3.0 Discovery & Job Resilience"]
        M4["4. RenegadeSwarm-Exclusive P2P Search & Discovery"]
        M5["5. Packaging State Hoisting & Job Resilience"]
        M3 --> M4 --> M5
    end

    subgraph v040 ["v0.4.0 Upstream Ingestion Mesh (Active)"]
        M6["6. CMM Upstream Fallback & Instant Swarm Ingestion"]
        M5 --> M6
    end
```
...
### Milestone 1: Scoped Companion Asset Harvesting & Local Directory Tree Discovery (v0.2.0)
...
- **Direct Sibling Harvesting**:
  - Enforces companion discovery strictly in the model's directory tree:
    - `<model_base_name>.sha256`: Plaintext SHA-256 hash.
    - `<model_base_name>.civitai.info` / `<model_base_name>.huggingface.info` / `<model_base_name>.info`: Complete JSON metadata payload.
    - `<model_base_name>.png` / `<model_base_name>.jpg` / `<model_base_name>.webp` / `<model_base_name>.preview.png`: Sibling preview image asset.
- **ComfyUI & CMM Workspace Awareness**:
  - Queries configured model roots from the RenegadeCMM SQLite bridge.
  - Automatically matches preview assets stored alongside checkpoints and LoRAs.
- **Embedded Workflow Metadata**:
  - Inspects companion images for embedded ComfyUI/A1111 prompt workflows (`tEXtparameters`, `tEXtworkflow`) and surfaces them during packaging.

---

### Milestone 2: Strict AI Model Payload Enforcement & Magic Byte Rejection (v0.2.0)
...
- **Manifest File Inspector**:
  - Parses incoming torrent metadata and manifest files before starting multi-part file allocation.
  - Validates all declared files against allowed AI extensions:
...
- **Magic Byte Enforcement**:
  - Prohibits executable signatures (`MZ`, `ELF`, `Mach-O`), generic archives (`RAR`, `7z`), and shell scripts (`#!`).
  - Rejects non-compliant torrents immediately with clear UI feedback.

---

### Milestone 3: Companion File Triplet Pre-Flight Handshake & Quarantine Verification (v0.2.0)
...
- **Pre-Flight Triplet Negotiation**:
  - Fetches the companion triplet first:
...
- **Pre-Download Verifier**:
  - Verifies against CivitAI (`/api/v1/model-versions/by-hash/:hash`) or Hugging Face APIs.
  - For unindexed or custom models (fine-tunes, custom LoRAs), executes local Ed25519 signature checks and Web of Trust scoring.
- **Atomic Quarantine & Promotion**:
  - Downloads weights to isolated `.part` staging files in Quarantine.
  - Computes full SHA-256 on completion; atomically promotes to library only when checksum and format validation pass.

---

### Milestone 4: RenegadeSwarm-Exclusive P2P Search & Discovery Engine (v0.3.0)

#### Problem Statement
Finding models on RenegadeSwarm previously required users to manually copy and paste Magnet URIs from external web pages. However, enabling open-ended torrent crawling or DHT scraping would open the floodgates to non-AI noise and arbitrary binaries. Swarm required an in-app, decentralized model search engine strictly isolated from standard BitTorrent noise.
...
- **Application-Specific BEP 10 Extension Handshake (`renegade_swarm_discovery_v1`)**:
  - Peers identify themselves during the BitTorrent extension handshake (BEP 10) by broadcasting custom capability dictionaries.
...
- **Signed Manifest & Metadata Broadcasts**:
  ...
  - Search hits are verified against local schemas before appearing in the search results UI.
...
### Milestone 5: Packaging State Hoisting & Background Job Persistence (v0.3.0)

#### Problem Statement
When a user prepared a model package in the **Package & Seed** view, entered metadata, and triggered hash calculation and manifest generation, navigating to another tab (e.g., *Dashboard* or *Bandwidth*) unmounted the React view. This unmounting wiped the form inputs and progress indicators from the UI even though the background worker was still actively computing the SHA-256 hash and assembling the manifest.

#### Architectural Implementation
- **Hoisted Packaging State**:
  - Moved packaging job states (`idle`, `hashing`, `validating`, `generating_manifest`, `seeding`, `completed`, `error`) out of local React component state (`SeederView.tsx`) and into a persistent main process service (`PackageJobManager`).
- **Main Process Job Manager (`src/main/engine/packageJobManager.ts`)**:
  - Tracks active packaging tasks by file path and job ID.
  - Streams chunk-level SHA-256 progress events over IPC (`swarm:packageProgress`).
  - Supports graceful cancellation tokens and cleanup handlers.
- **Resilient UI Binding**:
  - On mount, `SeederView` requests active job states via `window.renegadeSwarm.getActivePackagingJob()`.
  - Re-attaches progress bars, hash logs, and form fields when switching tabs.

---

### Milestone 6: RenegadeCMM Upstream Fallback & Instant Swarm Ingestion (Target: v0.4.0)
...
| :--- | :---: | :--- | :---: |
| **Milestone 1** | **v0.2.0** | Scoped Companion Asset Harvesting & Local Directory Tree Discovery | ✅ Implemented |
| **Milestone 2** | **v0.2.0** | Strict AI Model Payload Gatekeeping & Magic Byte Rejection | ✅ Implemented |
| **Milestone 3** | **v0.2.0** | Companion Triplet Pre-Flight Handshake & Quarantine Verification | ✅ Implemented |
| **Milestone 4** | **v0.3.0** | RenegadeSwarm-Exclusive P2P Model Search & Discovery Engine | ✅ Implemented |
| **Milestone 5** | **v0.3.0** | Packaging State Hoisting & Background Job Persistence | ✅ Implemented |
| **Milestone 6** | **v0.4.0** | RenegadeCMM Upstream Fallback (Hugging Face / Civitai) & Instant Seeding | 🔄 In Design |
